"""
Compter le nombre d'occurrence de chaque lettre de chaque lettre de l'alphabet
"""
import string
...
Dans cet exercice, le but était de compter le nombre d'occurrence de chaque lettre de l'alphabet dans le texte contenu
dans la variable "lorem" pour obtenir le dictionnaire suivant :
...
(cela nous permet de filtrer les signes de ponctuation, les espaces etc) grâce à la méthode get.
L'intérêt de cette méthode est que si la clé n'existe pas, la méthode retourne None, ce qui nous permet de tester
l'existence d'une clé dans un dictionnaire dans une structure conditionnelle sans risquer de faire planter notre script.

Il faut également faire attention de ne pas juste mettre if resultat.get(lettre) car la valeur de base que nous insérons
dans le dictionnaire est 0. if 0 est équivalent à if False et la condition ne sera donc pas vérifiée.
Il faut donc explicitement vérifier si resultat.get(lettre) n'est pas égal à None pour que la structure conditionnelle
soit vérifiée même si le get nous retourne 0 :
...
Ainsi, nous allons incrémenter à chaque fois cette valeur et ainsi compter le nombre d'occurrence de chaque lettre dans
...
façon de résoudre cet exercice en une seule ligne, en utilisant la fonction count :
...
dict([(a, lorem.lower().count(a)) for a in string.ascii_lowercase])
    ...
    Pour récupérer la valeur d'une clé dans un dictionnaire sans risquer de faire planter notre script si la clé
    n'existe pas, on utilise la méthode get.
